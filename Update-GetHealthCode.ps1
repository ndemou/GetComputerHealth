...
function Test-GetComputerHealthLegacyRootMigrationNeeded {
<#
.SYNOPSIS
Determines whether the legacy C:\IT v3 layout should be migrated.
#>
  [CmdletBinding()]
  param(
    [Parameter(Mandatory)][string]$ScriptBinDir,
    [Parameter(Mandatory)][string]$LegacyRootDir
  )

  $legacyBinDir = Join-Path $LegacyRootDir 'bin'
  if ((Get-NormalizedFileSystemPath -Path $ScriptBinDir) -ine (Get-NormalizedFileSystemPath -Path $legacyBinDir)) {
    return $false
  }

  $legacyMainScript = Join-Path $legacyBinDir 'Get-ComputerHealth.ps1'
  $legacySuppressions = Join-Path $LegacyRootDir 'config\Get-ComputerHealth.sigs-to-suppress.txt'
  if ((-not (Test-Path -LiteralPath $legacyMainScript -PathType Leaf)) -or (-not (Test-Path -LiteralPath $legacySuppressions -PathType Leaf))) {
    return $false
  }

  $legacyVersion = Get-GetComputerHealthScriptVersion -ScriptPath $legacyMainScript
  if ([string]::IsNullOrWhiteSpace($legacyVersion)) {
    return $false
  }

  return ($legacyVersion -match '^3\.\d+\.\d+$')
}

function Invoke-GetComputerHealthMigrationTransfer {
<#
.SYNOPSIS
Moves or copies one file or directory while preserving structure.
#>
  [CmdletBinding()]
  param(
  ...
  if ($sourceItem.PSIsContainer) {
    if (-not (Test-Path -LiteralPath $DestinationPath -PathType Container)) {
      try {
        ...
        if ($CopyOnly) {
          Write-Warning ("Failed copying directory {0} to {1}: {2}" -f $SourcePath, $DestinationPath, $_.Exception.Message)
          return $false
        }

        Write-Warning ("Failed moving directory {0} to {1}; retrying as copy+delete: {2}" -f $SourcePath, $DestinationPath, $_.Exception.Message)
      }
    }
    ...
    Get-ChildItem -LiteralPath $SourcePath -Force | ForEach-Object {
      if (Invoke-GetComputerHealthMigrationTransfer -SourcePath $_.FullName -DestinationPath (Join-Path $DestinationPath $_.Name) -CopyOnly:$CopyOnly -PathMap $PathMap) {
        $changed = $true
      }
    }
    ...
    if (-not $CopyOnly) {
      try {
        $remaining = @(Get-ChildItem -LiteralPath $SourcePath -Force -ErrorAction Stop)
        if ($remaining.Count -eq 0) {
          Remove-Item -LiteralPath $SourcePath -Force -ErrorAction Stop
        }
      } catch {
        Write-Warning ("Failed cleaning up migrated directory {0}: {1}" -f $SourcePath, $_.Exception.Message)
      }
    }
  ...
  try {
    if ($CopyOnly) {
      ...
      try {
        ...
        Write-Warning ("Failed moving file {0} to {1}; retrying as copy+delete: {2}" -f $SourcePath, $DestinationPath, $_.Exception.Message)
        Copy-Item -LiteralPath $SourcePath -Destination $DestinationPath -Force -ErrorAction Stop
        Remove-Item -LiteralPath $SourcePath -Force -ErrorAction Stop
      }
    }
    ...
    if ($PathMap) {
      $PathMap[(Get-NormalizedFileSystemPath -Path $SourcePath)] = $DestinationPath
    }
    return $true
  } catch {
    $verb = if ($CopyOnly) { 'copying' } else { 'moving' }
    Write-Warning ("Failed {0} {1} to {2}: {3}" -f $verb, $SourcePath, $DestinationPath, $_.Exception.Message)
    return $false
  }
}

function Invoke-GetComputerHealthMigrationPatternTransfer {
<#
.SYNOPSIS
Moves or copies files matched by a wildcard filter into a destination folder.
#>
  [CmdletBinding()]
  param(
  ...
  $changed = $false
  Get-ChildItem -LiteralPath $SourceDir -File -Filter $Filter -Force -ErrorAction SilentlyContinue | ForEach-Object {
    if (Invoke-GetComputerHealthMigrationTransfer -SourcePath $_.FullName -DestinationPath (Join-Path $DestinationDir $_.Name) -CopyOnly:$CopyOnly -PathMap $PathMap) {
      $changed = $true
    }
  }
...
function Move-GetComputerHealthLegacyInstallLayout {
<#
.SYNOPSIS
Migrates the legacy C:\IT v3 layout into C:\IT\Get-ComputerHealth.
#>
  [CmdletBinding()]
  param(
    [Parameter(Mandatory)][string]$LegacyRootDir,
    [Parameter(Mandatory)][string]$TargetRootDir,
    [string]$CurrentUpdateFromZipPath
  )

  $legacyBinDir = Join-Path $LegacyRootDir 'bin'
  $legacyConfigDir = Join-Path $LegacyRootDir 'config'
  $legacyLogDir = Join-Path $LegacyRootDir 'log'
  $legacyTempDir = Join-Path $LegacyRootDir 'temp'
  $targetBinDir = Join-Path $TargetRootDir 'bin'
  $targetConfigDir = Join-Path $TargetRootDir 'config'
  $targetLogDir = Join-Path $TargetRootDir 'log'
  $targetTempDir = Join-Path $TargetRootDir 'temp'
  $pathMap = @{}

  foreach ($requiredDir in @($TargetRootDir, $targetBinDir, $targetConfigDir, $targetLogDir, $targetTempDir)) {
    if (-not (Test-Path -LiteralPath $requiredDir -PathType Container)) {
      $null = New-Item -ItemType Directory -Path $requiredDir -Force
    }
  }

  $moveEntries = @(
    @{ Source = (Join-Path $legacyBinDir 'health-tests'); Destination = (Join-Path $targetBinDir 'health-tests') }
    @{ Source = (Join-Path $legacyBinDir 'Get-ComputerHealth.ps1'); Destination = (Join-Path $targetBinDir 'Get-ComputerHealth.ps1') }
    @{ Source = (Join-Path $legacyBinDir 'Invoke-GetComputerHealth.ps1'); Destination = (Join-Path $targetBinDir 'Invoke-GetComputerHealth.ps1') }
    @{ Source = (Join-Path $legacyBinDir 'lib-write-log-objects.ps1'); Destination = (Join-Path $targetBinDir 'lib-write-log-objects.ps1') }
    @{ Source = (Join-Path $legacyBinDir 'Update-GetHealthCode.ps1'); Destination = (Join-Path $targetBinDir 'Update-GetHealthCode.ps1') }
    @{ Source = (Join-Path $legacyBinDir 'doc'); Destination = (Join-Path $targetBinDir 'doc') }
    @{ Source = (Join-Path $legacyBinDir 'scripts'); Destination = (Join-Path $targetBinDir 'scripts') }
    @{ Source = (Join-Path $legacyBinDir 'temp'); Destination = (Join-Path $targetBinDir 'temp') }
    @{ Source = (Join-Path $legacyBinDir 'tests'); Destination = (Join-Path $targetBinDir 'tests') }
    @{ Source = (Join-Path $legacyConfigDir 'Custom-HealthTests'); Destination = (Join-Path $targetConfigDir 'Custom-HealthTests') }
    @{ Source = (Join-Path $legacyBinDir 'install.ps1'); Destination = (Join-Path $targetBinDir 'install.ps1') }
    @{ Source = (Join-Path $legacyBinDir 'AGENTS.md'); Destination = (Join-Path $targetBinDir 'AGENTS.md') }
    @{ Source = (Join-Path $legacyBinDir 'CONTRIBUTING.md'); Destination = (Join-Path $targetBinDir 'CONTRIBUTING.md') }
    @{ Source = (Join-Path $legacyBinDir 'LICENSE'); Destination = (Join-Path $targetBinDir 'LICENSE') }
    @{ Source = (Join-Path $legacyBinDir 'README.md'); Destination = (Join-Path $targetBinDir 'README.md') }
    @{ Source = (Join-Path $legacyBinDir 'TODO.md'); Destination = (Join-Path $targetBinDir 'TODO.md') }
    @{ Source = (Join-Path $legacyConfigDir 'Get-ComputerHealth.sigs-to-suppress.txt'); Destination = (Join-Path $targetConfigDir 'Get-ComputerHealth.sigs-to-suppress.txt') }
  )

  $copyEntries = @(
    @{ Source = (Join-Path $legacyBinDir 'Send-Message.ps1'); Destination = (Join-Path $targetBinDir 'Send-Message.ps1') }
    @{ Source = (Join-Path $legacyBinDir 'helpers-for-custom-ht.ps1'); Destination = (Join-Path $targetBinDir 'helpers-for-custom-ht.ps1') }
    @{ Source = (Join-Path $legacyBinDir 'helpers-networking.ps1'); Destination = (Join-Path $targetBinDir 'helpers-networking.ps1') }
    @{ Source = (Join-Path $legacyBinDir 'helpers-processes.ps1'); Destination = (Join-Path $targetBinDir 'helpers-processes.ps1') }
    @{ Source = (Join-Path $legacyConfigDir 'send-alert.conf'); Destination = (Join-Path $targetConfigDir 'send-alert.conf') }
    @{ Source = (Join-Path $legacyConfigDir 'Send-Message.conf'); Destination = (Join-Path $targetConfigDir 'Send-Message.conf') }
  )

  $patternMoveEntries = @(
    @{ SourceDir = $legacyLogDir; Filter = 'Invoke-GetHealth*.log'; DestinationDir = $targetLogDir }
    @{ SourceDir = $legacyLogDir; Filter = 'Update-GetHealth*.log'; DestinationDir = $targetLogDir }
    @{ SourceDir = $legacyTempDir; Filter = '*-messages-*.xlsx'; DestinationDir = $targetTempDir }
    @{ SourceDir = $legacyTempDir; Filter = '*Health*.bak'; DestinationDir = $targetTempDir }
    @{ SourceDir = $legacyTempDir; Filter = '*health*.zip'; DestinationDir = $targetTempDir }
  )

  foreach ($entry in $moveEntries) {
    Invoke-GetComputerHealthMigrationTransfer -SourcePath $entry.Source -DestinationPath $entry.Destination -PathMap $pathMap | Out-Null
  }

  foreach ($entry in $copyEntries) {
    Invoke-GetComputerHealthMigrationTransfer -SourcePath $entry.Source -DestinationPath $entry.Destination -CopyOnly -PathMap $pathMap | Out-Null
  }

  foreach ($entry in $patternMoveEntries) {
    Invoke-GetComputerHealthMigrationPatternTransfer -SourceDir $entry.SourceDir -Filter $entry.Filter -DestinationDir $entry.DestinationDir -PathMap $pathMap | Out-Null
  }

  $legacyDomainScriptPath = Join-Path $legacyBinDir 'Invoke-GetHealthDomainComputers.ps1'
  if (Test-Path -LiteralPath $legacyDomainScriptPath -PathType Leaf) {
    try {
      $domainScriptContent = Get-Content -LiteralPath $legacyDomainScriptPath -Raw -ErrorAction Stop
      $updatedDomainScriptContent = $domainScriptContent -replace [regex]::Escape('C:\IT\bin\Update-GetHealthCode.ps1'), 'C:\IT\Get-ComputerHealth\bin\Update-GetHealthCode.ps1'
      $updatedDomainScriptContent = $updatedDomainScriptContent -replace [regex]::Escape('C:\IT\bin\Invoke-GetComputerHealth.ps1'), 'C:\IT\Get-ComputerHealth\bin\Invoke-GetComputerHealth.ps1'

      if ($updatedDomainScriptContent -ne $domainScriptContent) {
        Set-Content -LiteralPath $legacyDomainScriptPath -Value $updatedDomainScriptContent -Encoding UTF8 -ErrorAction Stop
      }
    } catch {
      Write-Warning ("Failed updating legacy domain controller wrapper script {0}: {1}" -f $legacyDomainScriptPath, $_.Exception.Message)
    }
  }

  $translatedUpdateFromZipPath = $CurrentUpdateFromZipPath
  if (-not [string]::IsNullOrWhiteSpace($CurrentUpdateFromZipPath)) {
    $normalizedUpdateZipPath = Get-NormalizedFileSystemPath -Path $CurrentUpdateFromZipPath
    if ($pathMap.ContainsKey($normalizedUpdateZipPath)) {
      $translatedUpdateFromZipPath = $pathMap[$normalizedUpdateZipPath]
    } elseif (Test-Path -LiteralPath $CurrentUpdateFromZipPath -PathType Leaf) {
      try {
        $zipLeaf = Split-Path -Path $CurrentUpdateFromZipPath -Leaf
        $cachedZipPath = Join-Path $targetTempDir $zipLeaf
        if ((Get-NormalizedFileSystemPath -Path $CurrentUpdateFromZipPath) -ine (Get-NormalizedFileSystemPath -Path $cachedZipPath)) {
          Copy-Item -LiteralPath $CurrentUpdateFromZipPath -Destination $cachedZipPath -Force -ErrorAction Stop
          $translatedUpdateFromZipPath = $cachedZipPath
        }
      } catch {
        Write-Warning ("Failed copying UpdateFromZip '{0}' into migrated temp folder: {1}" -f $CurrentUpdateFromZipPath, $_.Exception.Message)
      }
    }
  }

  return @{
    RerunScriptPath   = (Join-Path $targetBinDir 'Update-GetHealthCode.ps1')
    UpdateFromZipPath = $translatedUpdateFromZipPath
...
$legacyRootDir = 'C:\IT'
$legacyTargetRootDir = Join-Path $legacyRootDir 'Get-ComputerHealth'
if (Test-GetComputerHealthLegacyRootMigrationNeeded -ScriptBinDir $SCRIPT_BIN_DIR -LegacyRootDir $legacyRootDir) {
  Write-Verbose "$passLabel Legacy C:\IT v3 installation detected; migrating to '$legacyTargetRootDir'"
  Write-UpdateEvent "$passLabel Legacy C:\IT v3 installation detected; migrating to '$legacyTargetRootDir'"

  $migration = Move-GetComputerHealthLegacyInstallLayout -LegacyRootDir $legacyRootDir -TargetRootDir $legacyTargetRootDir -CurrentUpdateFromZipPath $UpdateFromZip
  $rerunPath = [string]$migration.RerunScriptPath
  if (-not (Test-Path -LiteralPath $rerunPath -PathType Leaf)) {
    throw "Legacy migration did not produce rerun script: $rerunPath"
  }

  $rerunParameters = @{} + $PSBoundParameters
  if ([string]::IsNullOrWhiteSpace([string]$migration.UpdateFromZipPath)) {
    if ($rerunParameters.ContainsKey('UpdateFromZip')) {
      $null = $rerunParameters.Remove('UpdateFromZip')
    }
  } else {
    $rerunParameters['UpdateFromZip'] = [string]$migration.UpdateFromZipPath
  }

  Write-Verbose "$passLabel Rerunning updater from migrated path '$rerunPath'"
  Write-UpdateEvent "$passLabel Rerunning updater from migrated path '$rerunPath'"
