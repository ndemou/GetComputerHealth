...
.PARAMETER DevMode
After extracting the zip to the stage folder, replaces staged `install-gch.ps1`
...
function Enter-InstallMutex {
  ...
  try {
    ...
    if (-not $acquired) {
      throw 'Another install-gch.ps1 instance is already running for this bin folder.'
    }
...
function Resolve-SourcePlan {
  ...
  if ($requestedKind -eq 'GitHub') {
    ...
    Download-File -Uri $info.DownloadUri -DestinationPath $downloadPath -Headers @{ 'User-Agent' = 'install-gch.ps1' }
...
function Test-StagedPackage {
  ...
  $stagedInstaller = Join-Path $packageRoot 'install-gch.ps1'
  if (-not (Test-Path -LiteralPath $stagedInstaller -PathType Leaf)) {
    throw 'Package must contain install-gch.ps1 at package root.'
  }
...
function Invoke-StagedDeployment {
  ...
  foreach ($file in $allFiles) {
    ...
    if ($relative -ieq 'install-gch.ps1') { continue }
  ...
  $installerSource = Join-Path $validated.PackageRoot 'install-gch.ps1'
  $installerDest = Join-Path $BinPath 'install-gch.ps1'
  if (Test-FilesDifferent -SourcePath $installerSource -DestinationPath $installerDest) {
    ...
    Write-Log -Message 'Copied installer as final committed step: install-gch.ps1'
    $filesCopied++
  }
  else {
...
try {
  ...
  Write-Log -Message ('Starting install-gch.ps1. InternalStageRun={0}; Bin={1}' -f $InternalStageRun, $targetBin)
